...
<body>
    <nav class="flex justify-between flex items-center h-20 bg-[#219C90]">
        <a href="tabel_data.php" class="text-[32px] text-white font-bold ml-10">SIDINO</a>
    ...
    <div class="max-w-7xl mx-auto">
        <div class="relative overflow-x-hidden shadow-md sm:rounded-lg">
            <div class="p-3 bg-[#219C90] flex justify-between items-center">
                <!-- search bar -->
                <form class="relative mt-1" action="tabel_data.php" method="GET">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <form action="upload.php" method="POST" enctype="multipart/form-data" class="flex items-center">
                        <div>
                            ...
                            <input type="hidden" name="tipe" value="tabel_data">
    ...
    <script>
            ...
            $.get("get.php?path=" + path, function(data, status) {
                ...
                for (var i = 0; i < dir.length; i++) {
                    ...
                    if (dir_name.toLowerCase().includes(searchTerm) || dir_path.toLowerCase().includes(searchTerm)) {
                        html += `<tr class="bg-white border-b hover:bg-gray-50 cursor-pointer" data-type="dir" data-url="${dir_path}">
                <td class="w-4 p-4">
                ...
                <td scope="row" class="ss px-6 py-4 font-medium text-gray-900">
                    ${dir_path}
                </td>
            </tr>`;
                    }
                ...
                for (var i = 0; i < file.length; i++) {
                    ...
                    if (file_name.toLowerCase().includes(searchTerm) || file_path.toLowerCase().includes(searchTerm)) {
                        html += `<tr class="bg-white border-b hover:bg-gray-50" data-type="file" data-url="${file_path}">
                <td class="w-4 p-4">
                    <div onclick="event.stopPropagation()" class="flex items-center">
                ...
                $('#file-table tbody tr').on('click', function() {
        ...
        $('#btn-move').on('click', function() {
            const selectedItems = [];
            const moveDestination = $('#moveDestination').val();
            ...
            if (selectedItems.length > 0) {
                // Kirim permintaan AJAX ke server untuk memindahkan file atau folder
                $.ajax({
                    url: 'move.php',
                    data: {
                        items: selectedItems,
                    ...
                    success: function(data) {
                        ...
                        data = JSON.parse(data);
                        if (data.success) {
                            // Jika pemindahan berhasil, refresh konten dengan memanggil getAll
                            getAll(currentPath, "");
                        } else {
                ...
                alert("No files or folders selected for moving.");
            }
        });

        // Update destination list function
        function updateMoveDestinationList() {
            $.get("get.php", function(data, status) {
                data = JSON.parse(data);
                let dir = data.dir;
                let moveDestinationSelect = $('#moveDestination');

                moveDestinationSelect.empty();

                for (var i = 0; i < dir.length; i++) {
                    let dir_path = dir[i];
                    moveDestinationSelect.append($('<option>', {
                        value: dir_path,
                        text: dir_path
                    }));
                }
        ...
        // Panggil fungsi ini setiap kali Anda perlu memperbarui daftar tujuan pemindahan
        updateMoveDestinationList();
